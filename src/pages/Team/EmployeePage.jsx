  ...
  if (lowerName.includes("sham") || lowerName.includes("shamsk")) {
    return [
      ...
      },
      {
        ...
        title: "Dilse FM",
        description:
          "Social media algorithm consulting Graphic support Videography Market analysts Business consulting Marketing",
        icon: Briefcase,
      },
      {
        year: "Apr 2024",
      ...
      },
      {
        ...
        title: "Social Bureau",
        description:
          "CEO & Co-Founder of Social Bureau, I lead the world’s first API Marketing company — a revolutionary platform built to redefine how brands connect, communicate, and convert in the digital world.",
        icon: Trophy,
      },
      {
        year: "Jan 2026",
...
export const getDynamicMilestones = (achievements, slug = "") => {
  if (
    !achievements ||
  ...
  return sorted.map((ach) => {
    ...
    if (slug && ["alen", "alen-jacob"].includes(slug.toLowerCase())) {
      desc = desc.replace(/\bMD\b/g, "Managing Director");
    }
    return {
...
const attendanceData = generateFallbackAttendance(
...
const GlassCard = ({ children, className, variant = "default" }) => (
  <motion.div
    ...
    className={cn(
      variant === "purple" ? "glass-card-purple" : "glass-card",
      "p-4 md:p-6",
      className,
    )}
  >
...
const StatItem = ({ label, value, color }) => (
  <div className="flex flex-col items-center justify-center py-4 px-4 md:py-6 md:px-8 grow transition-all hover:bg-white/5 border border-white/5 md:border-y-0 md:border-l-0 md:border-r md:last:border-0">
    <span
      className={cn(
        "text-2xl md:text-4xl lg:text-5xl font-black mb-1 md:mb-2 drop-shadow-sm tracking-tight",
        color,
      )}
    >
      {value}
    </span>
    <span className="text-[9px] md:text-[11px] text-gray-400 font-bold tracking-[0.15em] md:tracking-[0.2em] uppercase text-center">
      {label}
    </span>
  </div>
);
...
const CircularProgress = ({ value, label, subLabel, color }) => {
  ...
  return (
    <div
      className="border border-white/10 rounded-[28px] p-6 flex flex-col items-center text-center flex-1 shadow-lg shadow-black/20"
      style={{
        background:
    ...
    >
      <span className="text-[10px] text-gray-400 font-bold uppercase tracking-[0.15em] mb-6">
        {label}
      </span>
  ...
  return (
    <div className="glass-card mb-4 p-5 hover:bg-white/10 transition-all cursor-pointer group relative">
      <div className="flex justify-between items-start mb-3">
        <span
          className={cn(
            "px-3 py-1 rounded-[30px] btn-typo uppercase",
            type === "INNOVATION"
              ? "bg-[#441649]/40 text-purple-400 border border-purple-500/30"
              : type === "CASE STUDY"
                ? "bg-[#102C1A]/48 text-green-400 border border-green-500/30"
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-gray-500 font-medium uppercase shrink-0">
            {date}
          </span>
          ...
          >
            ...
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="w-3.5 h-3.5 text-gray-400 group-hover:text-[#0099c2] transition-colors"
              viewBox="0 0 24 24"
              fill="none"
              stroke="currentColor"
              strokeWidth="2.5"
      ...
      <h3 className="text-sm font-semibold mb-2 group-hover:text-brand-purple transition-colors">
        {title}
      </h3>
      ...
      <div className="flex items-center gap-4 text-[10px] text-gray-500 font-medium">
        <span className="flex items-center gap-1 group-hover:text-brand-pink transition-colors">
          <Star className="w-3 h-3" /> {likes}
        </span>
  ...
  if (isSham) {
    return {
      member: {
        ...
        image:
          originalData?.member?.image ||
          "https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/socialbureau-media/images/Team/bulcmcbtguawhkw9f7oo.webp",
        cardImage:
          originalData?.member?.cardImage ||
          "https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/socialbureau-media/images/Team/dgox61jo9mebikejqe2i.webp",
        image1:
          originalData?.member?.image1 ||
          "https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/socialbureau-media/images/Team/cfg8edyz3kmmxjhqt98y.webp",
        description:
          originalData?.member?.description ||
          "Redefining how brands Attract, Pull & Influence — through data, systems, and relentless execution.",
        tagline:
          originalData?.member?.tagline ||
          "Founder & CEO · World's First API Marketing Consultant",
      ...
      clickup: {
        ...
        tasks: [
          {
            title: "API Attract-Pull Framework Release v3",
            listName: "API Marketing Strategy",
            folderName: "SUNTIPS",
            status: "complete",
            statusType: "closed",
            closedDateMs: Date.now() - 3600000,
          },
          {
            title: "Suntips Global Expansion Business Pitch",
            listName: "Global Brand Integration",
            folderName: "OPERATIONS",
            status: "complete",
            statusType: "closed",
            closedDateMs: Date.now() - 14400000,
          },
          {
            title: "Quantum AI Advertising Pipeline Strategy",
            listName: "AI Automation Pipelines",
            folderName: "TECHNOLOGY",
            status: "in progress",
            statusType: "active",
            updatedDateMs: Date.now() - 86400000,
          },
          {
            title: "Socialbureau-backend Microservices Workflow Audit",
            listName: "API Marketing Strategy",
            folderName: "SUNTIPS",
            status: "complete",
            statusType: "closed",
            closedDateMs: Date.now() - 172800000,
          },
        ],
      },
      user: {
        ...
        doj:
          originalData?.user?.doj ||
          originalData?.member?.user?.doj ||
          "2019-01-15",
        emp_id:
          originalData?.user?.emp_id ||
          originalData?.member?.user?.emp_id ||
          "SB-CEO-001",
        location:
          originalData?.user?.location ||
          originalData?.member?.user?.location ||
          "Kochi, Kerala",
        department:
          originalData?.user?.department ||
          originalData?.member?.user?.department ||
          "Leadership & Strategy",
        followers:
        ...
        hobbies: originalData?.user?.hobbies || [
          "Strategic Chess",
    ...
    return {
      member: {
        ...
        bgText: originalData?.member?.bgText || "MANAGING DIRECTOR",
        image:
          originalData?.member?.image ||
          "https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/images/Team/553ba2d1-f4d4-4333-b90b-b8f1a0f8639c.webp",
        cardImage:
          originalData?.member?.cardImage ||
          "https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/images/Team/d397c533-9dea-4cf2-913b-f54a453102a3.webp",
        image1:
          originalData?.member?.image1 ||
          "https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/images/Team/c5983092-1827-4fdb-a9a9-ec643d616919.webp",
        description:
          originalData?.member?.description ||
          "Spearheading regional corporate growth, strategic real estate developments, and business operations.",
        tagline:
          originalData?.member?.tagline ||
          "Managing Director · Real Estate & Strategy Expert",
        bgColor: originalData?.member?.bgColor || "#ff3358",
        socials: originalData?.member?.socials || {
      ...
      clickup: {
        ...
        tasks: [
          {
            title: "Skyline Builders Land Acquisition Proposal",
            listName: "Corporate Real Estate Expansion",
            folderName: "REAL ESTATE",
            status: "complete",
            statusType: "closed",
            closedDateMs: Date.now() - 7200000,
          },
          {
            title: "Muthoot Finance Strategic Alignment Term Sheet",
            listName: "Strategic Partnership Deals",
            folderName: "PARTNERSHIP",
            status: "complete",
            statusType: "closed",
            closedDateMs: Date.now() - 18000000,
          },
          {
            title: "SocialBureau Operations Restructuring Plan",
            listName: "Organizational Restructuring",
            folderName: "OPERATIONS",
            status: "in progress",
            statusType: "active",
            updatedDateMs: Date.now() - 90000000,
          },
          {
            title: "Skyline Builders Q2 Financial Audit Review",
            listName: "Corporate Real Estate Expansion",
            folderName: "REAL ESTATE",
            status: "complete",
            statusType: "closed",
            closedDateMs: Date.now() - 259200000,
          },
        ],
      },
      user: {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 max-w-4xl mx-auto flex flex-col items-center">
        ...
        >
          Architecting a{" "}
          <span className="text-[#C084FC] italic">Borderless</span> Influence
          Network
        </motion.h2>
        ...
        >
          SocialBureau operates beyond traditional borders, leveraging a hybrid
          mesh of digital influence and physical infrastructure.
        </motion.p>
    ...
    <section className="relative py-16 md:py-24 max-w-[1440px] mx-auto px-4 md:px-8 overflow-hidden z-10">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
        ...
        <div className="lg:col-span-6 order-2 lg:order-1">
          <div
            className="glass-card p-5 md:p-6 flex flex-col gap-5 md:gap-6 rounded-[28px] border border-white/10 shadow-2xl relative"
            style={{ backgroundColor: "rgba(15, 6, 25, 0.45)" }}
          >
            {/* Item 1 */}
            <div className="flex gap-4 md:gap-5 items-start">
              ...
              <div>
                <h3 className="text-xs font-bold text-white tracking-wide mb-1 uppercase">
                  Architectural Governance
                </h3>
                <p className="text-[11px] text-gray-400 leading-relaxed">
                  Implementing multi-layered communication frameworks that
                  bridge central policy with regional execution.
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-4 md:gap-5 items-start">
              ...
              <div>
                ...
                <p className="text-[11px] text-gray-400 leading-relaxed">
                  Utilizing socio-political data modeling to anticipate
                  narrative shifts and maintain platform stability.
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-4 md:gap-5 items-start">
              ...
              <div>
                ...
                <p className="text-[11px] text-gray-400 leading-relaxed">
                  Ensuring 24/7 operational continuity across sovereign
                  infrastructure with zero-latency protocols.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-6 flex flex-col gap-5 md:gap-6 order-1 lg:order-2">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-white tracking-tight uppercase leading-[1.1] mb-5">
              Strategic{" "}
              <span className="text-[#C084FC] italic">Operations</span>
            </h2>
            <p className="text-xs sm:text-sm text-gray-400 leading-relaxed">
              Alen Jacob’s operational philosophy is rooted in technical
              precision and strategic foresight. As Managing Director, he
              oversees the intricate web of digital assets and human
              intelligence that defines SocialBureau's competitive edge in the
              global arena.
            </p>
          </div>
          ...
          <div
            className="glass-card-purple p-4 rounded-[16px] border-l-4 border-l-[#A855F7] border-white/5 shadow-xl"
            style={{ backgroundColor: "rgba(15, 6, 25, 0.45)" }}
          >
            <p className="text-xs sm:text-sm font-bold italic text-white/90 leading-relaxed">
              "Efficiency is not just about speed,it's about the integrity of
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6 md:gap-8">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-white tracking-tight uppercase leading-[1.1] mb-5">
              Attract–Pull–
              <span className="text-[#C084FC] italic">Influence</span>
            </h2>
            <p className="text-xs sm:text-sm text-gray-400 leading-relaxed max-w-3xl">
              Sham's proprietary framework is the backbone of SocialBureau's
              success. It orchestrates digital ecosystems by merging human
              psychology with technical API automation.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-3">
            {/* Card 1: Attract */}
            <div
              className="glass-card p-4 md:p-5 rounded-[16px] border-l-4 border-l-[#EC4899] border-white/5 flex gap-4 items-center shadow-xl transition-all hover:scale-[1.005]"
              style={{ backgroundColor: "rgba(15, 6, 25, 0.45)" }}
            >
              ...
              <div>
                <h3 className="text-xs font-bold text-white tracking-wide mb-0.5 uppercase">
                  Attract
                </h3>
            ...
            {/* Card 2: Pull */}
            <div
              className="glass-card p-4 md:p-5 rounded-[16px] border-l-4 border-l-[#A855F7] border-white/5 flex gap-4 items-center shadow-xl transition-all hover:scale-[1.005]"
              style={{ backgroundColor: "rgba(15, 6, 25, 0.45)" }}
            >
            ...
            {/* Card 3: Influence */}
            <div
              className="glass-card p-4 md:p-5 rounded-[16px] border-l-4 border-l-[#06B6D4] border-white/5 flex gap-4 items-center shadow-xl transition-all hover:scale-[1.005]"
              style={{ backgroundColor: "rgba(15, 6, 25, 0.45)" }}
            >
              ...
              <div>
                ...
                <p className="text-[11px] text-gray-400">
                  Nurturing long-term brand advocates with personalized
                  automation.
                </p>
              </div>
            </div>
        ...
        <div className="lg:col-span-5 flex flex-col gap-4 md:gap-5">
          <div
            className="glass-card p-5 md:p-6 rounded-[28px] border border-white/10 shadow-2xl relative flex flex-col gap-5 overflow-hidden"
            style={{ backgroundColor: "rgba(15, 6, 25, 0.45)" }}
          >
            ...
            <div className="bg-black/50 border border-white/5 rounded-2xl p-4 font-mono text-[10px] leading-relaxed text-[#06B6D4] space-y-1 shadow-inner select-none relative overflow-hidden text-left">
              <div>
                ...
                <span className="text-yellow-400">APIMarketing</span>();
              </div>
              <div>
                <span className="text-blue-400">framework</span>.
              ...
              <div className="pl-4">
                <span className="text-gray-400">attract:</span>{" "}
                <span className="text-green-300">"API_TOUCHPOINTS"</span>,
              </div>
              <div className="pl-4">
                <span className="text-gray-400">pull:</span>{" "}
                <span className="text-green-300">"AUTOMATED_VALUE_LOOPS"</span>,
              </div>
              <div className="pl-4">
                <span className="text-gray-400">influence:</span>{" "}
                <span className="text-green-300">
                  "PERSONALIZED_CONVERSIONS"
                </span>
              </div>
              <div>{"}"});</div>
              <div className="pt-2 text-green-400/80 animate-pulse">
              ...
              {[
                ...
                <React.Fragment key={stat.label}>
                  <div className="flex flex-col items-center">
                    ...
                    <span className="text-[7px] text-gray-500 font-bold tracking-wider uppercase mt-1">
                      {stat.label}
                    </span>
                  </div>
...
const getCompanyLogo = (title) => {
  ...
  if (t.includes("business bureau") || t.includes("businessbureau")) {
    return "/assets/team page/Businessbureau_logo.png";
  }
  if (t.includes("social bureau") || t.includes("socialbureau")) {
    return "/assets/team page/Socialbureau_logo.png";
  }
  if (t.includes("realty bureau") || t.includes("realtybureau") || t.includes("realitybureau") || t.includes("reality bureau")) {
    return "/assets/team page/realityBureau.png";
  }
  if (t.includes("skway") || t.includes("skyway")) {
    return "/assets/team page/skway.png";
  }
  if (t.includes("trillion")) {
    return "/assets/team page/trillionedition.webp";
  }
  if (t.includes("tommy hilfiger")) {
    return "/assets/team page/Tommy_Hilfiger.png";
  }
  return null;
};

const getCompanyWebsite = (title) => {
  if (!title) return null;
  const t = title.toLowerCase();
  if (t.includes("business bureau") || t.includes("businessbureau")) {
    return "https://www.businessbureau.in/";
  }
  if (t.includes("social bureau") || t.includes("socialbureau")) {
    return "https://www.socialbureau.in/";
  }
  if (t.includes("realty bureau") || t.includes("realtybureau") || t.includes("realitybureau") || t.includes("reality bureau")) {
    return "https://www.realtybureau.in/";
  }
  if (t.includes("trillion")) {
    return "https://www.trillionedition.com/";
  }
  if (t.includes("tommy hilfiger")) {
    return "https://tommyhilfiger.nnnow.com/";
  }
  return null;
};
...
const SHAM_LOGOS = [
  {
    name: "Reporter TV",
    icon: "data:image/png;base64,",
  },
  {
    name: "Lakshya",
    icon: "https://api.lakshyacommerce.com/uploads/backend/setting/1714646008734logo.png",
  },
  {
    name: "Dilse FM",
    icon: "https://yt3.googleusercontent.com/RS4KMxrEwCPXSvbTyH9pdMFGobFRPb584qiiwfsbRmvZAFxyjNAuuDOPAeoZ07NV-X8ctfauXg=s160-c-k-c0x00ffffff-no-rj",
  },
  {
    name: "Al Madina UAE",
...
const EmployeePage = () => {
  ...
  const [coverUrl, setCoverUrl] = useState(
    "https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/TeamPage/syudjvadmoda2nj1albg.png",
  );
    ...
    ? {
        member: {
          ...
          image:
            ...
            "https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/socialbureau-media/images/Team/bulcmcbtguawhkw9f7oo.webp",
          cardImage:
            matchedMember.cardImage ||
            "https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/socialbureau-media/images/Team/dgox61jo9mebikejqe2i.webp",
          image1:
            matchedMember.image1 ||
            matchedMember.image ||
            "https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/TeamPage/Rectangle%2098.png",
          description:
            matchedMember.description ||
            `Specializing in ${matchedMember.role || "consulting"} — through data, systems, and relentless execution.`,
          tagline:
            matchedMember.tagline ||
            `${matchedMember.role || "Consultant"} · Team Member`,
  ...
  useEffect(() => {
    const fetchMemberDetails = async () => {
      try {
        ...
        console.error(
          "Error fetching employee details. Loading visual mock mode.",
          err,
        );
        setData(fallbackData);
        setIsDemoMode(true);
  ...
  const getDeduplicatedDepartment = (categories) => {
    ...
    formatted.forEach((item) => {
      if (item === "Leadership & Strategy") {
        if (
          !uniqueWords.includes("Leadership") &&
          !uniqueWords.includes("Strategy")
        ) {
    ...
    return (
      <g>
        ...
        <circle
          ...
          r={14}
          fill="none"
          stroke={stroke || "rgba(255,255,255,0.08)"}
          strokeWidth={2}
        />
      </g>
    );
  ...
  const milestones =
    user && Array.isArray(user.achievements) && user.achievements.length > 0
      ? getDynamicMilestones(user.achievements, slug)
  ...
  const isAlenOrSham = ["alen-jacob", "alen", "shamsk", "sham-sk"].includes(
    (slug || "").toLowerCase(),
  );
  const isSham = ["shamsk", "sham-sk"].includes((slug || "").toLowerCase());
  const showStaticFallback = isDemoMode;

  const hasInnovations =
  ...
  const resumeData = {
    ...
    experience:
      Array.isArray(user.achievements) && user.achievements.length > 0
        ? user.achievements.map((ach) => ({
            ...
            description: [
              ach.description ||
                "Achieved milestone goals and strategic company wins.",
            ],
          }))
        : [
            {
    ...
    projects:
      Array.isArray(user.workShowcase) && user.workShowcase.length > 0
      ...
      Array.isArray(user.certifications) && user.certifications.length > 0
        ...
        : Array.isArray(user.achievements)
          ? user.achievements.map(
              (ach) => `${ach.title} (${ach.date || "2026"})`,
            )
          : [],
    languages: ["English", "Malayalam"],
    softSkills: Array.isArray(user.hobbies)
      ? user.hobbies
      : ["Innovation", "Collaboration", "Execution"],
  };

  const hoursLogged =
    clickup && !clickup.error && typeof clickup.totalHours === "number"
      ? Number(clickup.totalHours).toFixed(1)
      : "0.0";
  const tasksCompleted =
    clickup && !clickup.error && typeof clickup.worksDone === "number"
      ? clickup.worksDone
      : 0;
  const totalTasks =
    clickup && !clickup.error && typeof clickup.totalTasks === "number"
      ? clickup.totalTasks
      : tasksCompleted;

  const activeActivityData =
    clickup && clickup.activityData ? clickup.activityData : activityData;
  const activeTaskCompletionData =
    clickup && clickup.taskCompletionData
      ? clickup.taskCompletionData
      : taskCompletionData;
  const activeWorkingHoursData =
    clickup && clickup.workingHoursData
      ? clickup.workingHoursData
      : workingHoursData;
  const activeAttendanceData =
    clickup && clickup.attendanceData
      ? clickup.attendanceData
      : generateFallbackAttendance(selectedYear, selectedMonth);
  const activeEfficiencyScoreData =
    clickup && clickup.efficiencyScoreData
      ? clickup.efficiencyScoreData
      : efficiencyScoreData;

  const maxScoreVal = Math.max(
    ...activeEfficiencyScoreData.map((d) =>
      Math.max(d.efficiency || 0, d.delivery || 0, d.csat || 0),
    ),
  );
  const yTicks =
    maxScoreVal > 50 ? [0, 20, 40, 60, 80, 100] : [0, 10, 20, 30, 40, 50];
  const yDomain = maxScoreVal > 50 ? [0, 100] : [0, 50];

  const attendancePresent = activeAttendanceData.filter(
    (d) => d.status === "present",
  ).length;
  const attendanceHalf = activeAttendanceData.filter(
    (d) => d.status === "half",
  ).length;
  const attendanceLeave = activeAttendanceData.filter(
    (d) => d.status === "leave",
  ).length;
  const attendanceHoliday = activeAttendanceData.filter(
    (d) => d.status === "holiday",
  ).length;
  const attendanceBalance = activeAttendanceData.filter(
    (d) => d.status === "upcoming",
  ).length;
  ...
  if (clickup && Array.isArray(clickup.tasks) && clickup.tasks.length > 0) {
    ...
    clickup.tasks.forEach((task) => {
      ...
      const isClosed =
        ...
        ["closed", "complete", "done"].includes(task.status?.toLowerCase());
      if (isClosed) {
    ...
    liveProjects = Object.values(projectMap).map((proj, idx) => {
      const progress =
        proj.total > 0 ? Math.round((proj.closed / proj.total) * 100) : 0;
      return {
        title: proj.title,
        label: proj.label,
        progress: progress,
    ...
    liveProjects = [
      {
        title: "Kerala Election 2026",
        label: "NEWS MALAYALAM",
        progress: 78,
        color: "bg-red-500",
      },
      {
        title: "API Marketing v2",
        label: "SUNTIPS",
        progress: 62,
        color: "bg-pink-500",
      },
      {
        title: "ClickUp Architecture",
        label: "WORKFLOW",
        progress: 85,
        color: "bg-blue-500",
      },
  ...
  if (clickup && Array.isArray(clickup.tasks) && clickup.tasks.length > 0) {
    ...
    recentActivities = sortedTasks.slice(0, 4).map((task, idx) => {
      ...
      const timeMs =
        task.closedDateMs || task.updatedDateMs || task.createdDateMs;
      const timeStr = formatRelativeTime(timeMs);
      const color = isClosed
        ? "bg-green-500"
        : activityColors[(idx + 1) % activityColors.length];
    ...
    recentActivities = [
      {
        label: "Completed",
        project: '"Election Dashboard Live"',
        time: "2 hours ago",
        color: "bg-green-500",
      },
      {
        label: "Started",
        project: '"Suntips Q3 Campaign"',
        time: "yesterday",
        color: "bg-red-500",
      },
      {
  ...
  const getBadgeStyles = (badgeType) => {
    ...
    switch (type) {
      case "MILESTONE":
        return {
          color: "bg-red-600",
          badgeColor: "bg-red-950/30 text-red-700 border-red-900/30",
        };
      case "INNOVATION":
        return {
          color: "bg-purple-600",
          badgeColor: "bg-purple-950/30 text-purple-700 border-purple-900/30",
        };
      case "CLIENT WIN":
      case "CLIENT_WIN":
        return {
          color: "bg-green-600",
          badgeColor: "bg-green-950/30 text-green-700 border-green-900/30",
        };
      case "PARTNERSHIP":
        return {
          color: "bg-yellow-400",
          badgeColor: "bg-yellow-950/30 text-yellow-700 border-yellow-900/30",
        };
      case "ACHIEVEMENT":
        return {
          color: "bg-cyan-500",
          badgeColor: "bg-cyan-950/30 text-cyan-700 border-cyan-900/30",
        };
      case "LAUNCH":
        return {
          color: "bg-pink-600",
          badgeColor: "bg-pink-950/30 text-pink-700 border-pink-900/30",
        };
      default:
        return {
          color: "bg-cyan-500",
  ...
  return (
    <div className="min-h-screen selection:bg-brand-purple/30 text-white relative overflow-hidden font-sans">
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(180deg, #441649 0%, #160F2C 45%, #2A1440 75%, #20133C 100%)",
        }}
      >
        ...
        <div
          ...
          style={{
            backgroundImage:
              'url("https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/TeamPage/image%20(40)%20(1).png")',
          }}
        />
      ...
      {isDemoMode && (
        <div className="bg-[#CB1387]/10 border-b border-[#CB1387]/20 text-center py-2 relative z-50 text-xs font-semibold text-[#CB1387] tracking-wider uppercase flex items-center justify-center gap-2">
          <Sparkles className="w-4.5 h-4.5 animate-spin" /> Interactive Preview
          Mode: Sham SK Profile Design
        </div>
      )}
            ...
            >
              <div className="w-[280px] h-[280px] md:w-[320px] md:h-[320px] rounded-[40px] overflow-hidden relative border border-white/10 shadow-2xl bg-black">
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-10 select-none z-0">
                  <span className="text-8xl font-black text-white leading-none rotate-90 scale-150 uppercase">
                    {member.name}
                  </span>
                </div>
                <img
          ...
          {!isAlenOrSham && (
            ...
            >
              ...
              <StatItem
                ...
                value={
                  typeof tasksCompleted === "number"
                    ? tasksCompleted
                    : fallbackData.clickup.worksDone || "0"
                }
                color="text-purple-500"
              />
              <StatItem
                label="Hours/Mo"
                value={hoursLogged ? `${hoursLogged}H` : "0.0H"}
                color="text-blue-500"
              />
              <StatItem
                label="Efficiency"
                value={
                  clickup && typeof clickup.efficiency === "number"
                    ? `${clickup.efficiency}%`
                    : user.efficiency || "94%"
                }
              ...
              <StatItem
                ...
                value={
                  user.doj
                    ? `${Math.max(1, new Date().getFullYear() - new Date(user.doj).getFullYear())}YR`
                    : user.tenure || "1YR"
                }
              ...
              <StatItem
                label="Rating"
                value={user.rating ? `${user.rating}★` : "5.0★"}
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            <div className={isSham ? "md:col-span-6" : "md:col-span-7"}>
              ...
              >
                ...
                <div className="relative mt-12 overflow-x-auto pt-6 pb-4 timeline-scrollbar select-none">
                  <style>{`
        .timeline-scrollbar {
      ...
      `}</style>
                  <div
                    className="relative min-w-[var(--slider-min-w)] md:min-w-0 min-h-[90px] px-6"
                    style={{
                      "--slider-min-w": `${milestones.length * 80 + 48}px`,
                    }}
                  >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8">
                  <div className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
                    <TrendingUp size={22} style={{ color: member.bgColor }} />
                    <h4 className="text-2xl font-bold mt-3 text-white">
                      {isSham ? "300%" : "100%"}
                    </h4>
                    <p className="text-gray-400 text-sm">Growth Delivered</p>
                  </div>

                  <div className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
                    <Users size={22} style={{ color: member.bgColor }} />
                    <h4 className="text-2xl font-bold mt-3 text-white">
                      {isSham ? "120+" : "10+"}
                    </h4>
                    <p className="text-gray-400 text-sm">Brands Managed</p>
                  </div>

                  <div className="rounded-xl border border-white/10 bg-white/[0.03] p-5">
                    <Briefcase size={22} style={{ color: member.bgColor }} />
                    <h4 className="text-2xl font-bold mt-3 text-white">
                      {isSham ? "500+" : "50+"}
                    </h4>
                    <p className="text-gray-400 text-sm">
                      {isSham ? "Campaigns Delivered" : "Projects Delivered"}
                    </p>
                  </div>
                </div>
              </GlassCard>
            ...
            <div
              className={cn(
                isSham ? "md:col-span-6" : "md:col-span-5",
                "flex flex-col gap-4",
              )}
            >
              {careerTimelineData.map((item, idx) => {
                const logoUrl = getCompanyLogo(item.label);
                const websiteUrl = getCompanyWebsite(item.label);
                const cardContent = (
                  <GlassCard className="p-3 flex items-start gap-3 group relative cursor-pointer h-full">
                    <div
                      className="shrink-0 w-12 h-12 rounded-lg flex items-center justify-center relative overflow-hidden"
                      style={{
                        background: `${member.bgColor || "#A855F7"}22`,
                        border: "1px solid rgba(255,255,255,0.04)",
                      }}
                    >
                      {logoUrl ? (
                        <>
                          <div className="transition-all duration-300 transform scale-100 opacity-100 group-hover:scale-0 group-hover:opacity-0 flex items-center justify-center w-full h-full">
                            {item.icon ? (
                          ...
                          <div className="absolute inset-0 transition-all duration-300 transform scale-50 opacity-0 group-hover:scale-100 group-hover:opacity-100 flex items-center justify-center p-1.5 bg-black/40">
                            <img
                              src={logoUrl}
                              alt={item.label}
                ...
                if (websiteUrl) {
                  return (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="block no-underline text-inherit hover:scale-[1.01] transition-all duration-300"
                    >
                      {cardContent}
                    </a>
                  );
      ...
      >
        {isSham && (
          <div className="lg:col-span-12 sham-logos-wrapper mb-2">
            <GlassCard variant="purple" className="sham-logos-container">
              <style>{`
                .sham-logo-item {
                  filter: grayscale(1) opacity(0.45) contrast(1.1);
                  transition: all 0.5s cubic-bezier(0.4, 0, 0.2, 1);
                }
                .sham-logos-container:hover .sham-logo-item {
                  filter: grayscale(0) opacity(1) contrast(1);
                }
                .sham-logo-item:hover {
                  transform: scale(1.1);
                }
              `}</style>
              <div className="flex flex-col lg:flex-row items-center justify-between gap-6 py-3 px-2">
                ...
                <div className="w-full flex flex-wrap items-center justify-center lg:justify-end gap-x-8 lg:gap-x-10 gap-y-4">
                  {SHAM_LOGOS.map((logo, i) => (
                    <div
                      key={i}
                    ...
                    >
                      <img
                        ...
                        alt={logo.name}
                        title={logo.name}
                        className="sham-logo-item max-h-full max-w-[130px] object-contain"
                      />
                    </div>
                  ))}
                </div>
        ...
        >
                    ...
                    {[
                      {
                        ...
                        val:
                          clickup && typeof clickup.efficiency === "number"
                            ? `${clickup.efficiency}%`
                            : user.efficiency || "94%",
                        color: "border-white/5 text-accent-red bg-white/2",
                      },
                      {
                        label: "ON-TIME",
                        val:
                          clickup && typeof clickup.onTime === "number"
                            ? `${clickup.onTime}%`
                            : user.onTime || "91%",
                        color: "border-white/5 text-accent-green bg-white/2",
                      },
                      {
                        label: "CSAT",
                        val:
                          clickup && typeof clickup.csat === "number"
                            ? `${clickup.csat}%`
                            : user.csat || "98%",
                        color: "border-white/5 text-brand-purple bg-white/2",
                      },
                      {
                        label: "TASKS/MO",
                        val:
                          typeof tasksCompleted === "number"
                            ? tasksCompleted
                            : fallbackData.clickup.worksDone || "47",
                        color: "border-white/5 text-brand-blue bg-white/2",
                      },
                    ].map((item) => (
                      <div
                        key={item.label}
                      ...
                      >
                        <div className="text-[14px] font-bold mb-1 tracking-tighter">
                          {item.val}
                        </div>
                        <div className="text-[9px] font-semibold tracking-widest opacity-60 uppercase">
                          {item.label}
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="h-48">
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        ...
                        />
                        <Tooltip
                          content={<CustomTooltip color="#F43F5E" />}
                          cursor={{
              ...
              <div className="md:col-span-6">
                <GlassCard variant="purple">
                  <div className="mb-6">
                    ...
                    />
                    <div className="text-3xl font-black text-accent-red tracking-tighter -mt-4">
                      {totalTasks} TASKS
                    </div>
                  </div>
                ...
                <GlassCard variant="purple">
                  <div className="mb-6">
                    <SectionTitle
                      ...
                      barColor="bg-brand-purple"
                    />
                    <div className="text-3xl font-black text-brand-purple tracking-tighter -mt-4">
                      {hoursLogged || "186"} HRS
                    </div>
                  </div>
                  <div className="h-48 mt-8">
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        ...
                        />
                        <Tooltip
                          content={<CustomTooltip color="#A855F7" />}
                          cursor={{
                            fill: "rgba(255, 255, 255, 0.03)",
                            radius: [6, 6, 0, 0],
                          }}
              ...
              <div className="md:col-span-12">
                <div className="bg-card-dark border border-white/5 rounded-[32px] p-4 md:p-8">
                  ...
                  <div className="h-80">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart
                        data={activeEfficiencyScoreData}
                ...
                >
                    ...
                    {attendanceLoading && (
                      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm z-20 rounded-2xl flex items-center justify-center border border-white/5">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <span className="text-[10px] font-bold tracking-widest text-purple-300 uppercase animate-pulse">
                            Syncing Time Log
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="flex flex-wrap gap-5 mt-10 mb-10">
                    {[
                      {
                        l: "Leave",
                        c: "bg-gradient-to-br from-red-500 to-red-700",
                      },
                      {
                        l: "Present",
                        c: "bg-gradient-to-br from-green-600 to-green-800",
                      },
                      {
                        l: "Half Day",
                      ...
                      {
                        l: "Public Holiday",
                        c: "bg-gradient-to-br from-purple-600 to-purple-800 border border-purple-400/20 shadow-[0_0_10px_rgba(168,85,247,0.3)]",
                      },
                      { l: "Weekend", c: "bg-[#112240]" },
                      {
                        l: "Upcoming",
                        c: "bg-gradient-to-br from-[#0099c2]/20 to-[#0099c2]/5 border border-[#0099c2] shadow-[0_0_10px_rgba(0,153,194,0.3)]",
                      },
                    ].map((i) => (
                      <div key={i.l} className="flex items-center gap-2">
                        <div
                          className={cn(
                            "w-3.5 h-3.5 rounded-[4px] shadow-sm",
                            i.c,
                          )}
                        />
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    {[
                      {
                        l: "PRESENT",
                        v: attendancePresent.toString(),
                        c: "text-green-500",
                      },
                      {
                        l: "HALF DAY",
                        v: attendanceHalf.toString(),
                        c: "text-orange-500",
                      },
                      {
                        l: "LEAVE",
                        v: attendanceLeave.toString(),
                        c: "text-red-500",
                      },
                      {
                        l: "HOLIDAY",
                        v: attendanceHoliday.toString(),
                        c: "text-purple-500",
                      },
                      {
                        l: "BALANCE",
                        v: attendanceBalance.toString(),
                        c: "text-cyan-500",
                      },
                    ].map((i) => (
                      <div
                        key={i.l}
                        className="bg-white/2 border border-white/5 rounded-2xl p-4 flex flex-col items-center justify-center gap-0.5 hover:bg-white/5 transition-colors"
                      >
                        <span
                          className={cn("text-lg font-black leading-none", i.c)}
                        >
                          {i.v}
                        </span>
              ...
              <div className="md:col-span-12 grid grid-cols-1 md:grid-cols-3 gap-6">
                <CircularProgress
                  value={
                    clickup && typeof clickup.efficiency === "number"
                      ? clickup.efficiency
                      : Number(user.efficiency?.replace("%", "")) || 94
                  }
                ...
                <CircularProgress
                  value={
                    clickup && typeof clickup.onTime === "number"
                      ? clickup.onTime
                      : Number(user.onTime?.replace("%", "")) || 91
                  }
                  label="ON-TIME DELIVERY"
                  subLabel="Delivery Score"
                  color="#22C55E"
                />
                <CircularProgress
                  value={
                ...
                <div className="bg-card-dark border border-white/5 rounded-[32px] p-4 md:p-10 overflow-visible">
                  ...
                  <div className="space-y-3 pb-6 md:pb-10 border-b border-white/5 mb-6 md:mb-8">
                    {(() => {
                      ...
                      return ["MON", "TUE", "WED", "THU", "FRI", "SAT"].map(
                        (day) => {
                          const dayData =
                            clickup &&
                            clickup.heatmapData &&
                          ...
                          return (
                            ...
                            >
                              <span className="text-[11px] text-gray-600 w-8 md:w-10 font-bold uppercase tracking-widest">
                                {day}
                              </span>
                                ...
                                {dayData.map((level, i) => (
                                  ...
                                  >
                                    ...
                                    <div
                                      ...
                                      style={{
                                        background:
                                          "linear-gradient(135deg, rgba(20,10,35,0.97) 0%, rgba(10,5,20,0.99) 100%)",
                                        boxShadow:
                  ...
                  {(user && Array.isArray(user.tools) && user.tools.length > 0
                    ...
                    <div key={exp.name} className="space-y-3 group">
                      <div className="flex justify-between items-center">
                        <span className="text-xs font-semibold text-gray-300 uppercase tracking-[0.15em] group-hover:text-white transition-colors">
                          {exp.name}
                        </span>
                      </div>
          ...
          {user.education && user.education.length > 0 && (
            <div className="md:col-span-12">
              <GlassCard
                variant="purple"
                className="!p-0 bg-[#0B0118] border-white/5"
              >
                <div className="p-8 border-b border-white/5">
                  <div className="flex items-center gap-3">
                    <div className="w-[3px] h-4 bg-brand-pink" />
                    <h2 className="text-[10px] font-bold tracking-[0.25em] uppercase text-gray-500">
                      EDUCATION
                    </h2>
                  </div>
                </div>
                  ...
                  {user.education.map((edu, i) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="text-sm font-bold text-white tracking-tight mb-0.5">
                          {edu.degree}
                        </h4>
                        ...
                        {(edu.year || edu.grade) && (
                          <div className="flex items-center gap-3 mt-1.5">
                            ...
                            {edu.grade && (
                              <span className="text-[9px] text-brand-pink/70 font-bold tracking-widest uppercase bg-brand-pink/10 px-2 py-0.5 rounded-full">
                                {edu.grade}
                              </span>
                            )}
                          </div>
                          ...
                          <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                            ...
                            {cert.credentialUrl && (
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-[9px] text-brand-pink font-bold tracking-widest uppercase hover:underline flex items-center gap-1"
                              >
                                View Credential →
                              </a>
                            )}
          ...
          {((user &&
            ...
            (showStaticFallback && !isAlenOrSham)) && (
            <div className="md:col-span-12">
              <GlassCard
                variant="purple"
                className="!p-0 bg-[#0B0118] border-white/5"
              >
                ...
                <div className="divide-y divide-white/5">
                  ...
                  user.achievements.length > 0
                    ...
                    : [
                        {
                          title: "Founded Social Bureau",
                          desc: "Kerala's first API Marketing agency",
                          date: "JAN 2019",
                          badge: "MILESTONE",
                          color: "bg-red-600",
                          badgeColor:
                            "bg-red-950/30 text-red-700 border-red-900/30",
                        },
                        {
                          title: "Coined API Marketing",
                          desc: "Attract - Pull - Influence framework launch",
                          date: "MAR 2020",
                          badge: "INNOVATION",
                          color: "bg-purple-600",
                          badgeColor:
                            "bg-purple-950/30 text-purple-700 border-purple-900/30",
                        },
                        {
                          title: "First News Channel Client",
                          desc: "NwsTamil24x7 digital growth strategy",
                          date: "AUG 2021",
                          badge: "CLIENT WIN",
                          color: "bg-green-600",
                          badgeColor:
                            "bg-green-950/30 text-green-700 border-green-900/30",
                        },
                        {
                          title: "ClickUp India Partner",
                          desc: "Official ClickUp reselling partnership",
                          date: "FEB 2022",
                          badge: "PARTNERSHIP",
                          color: "bg-yellow-400",
                          badgeColor:
                            "bg-yellow-950/30 text-yellow-700 border-yellow-900/30",
                        },
                        {
                          title: "13-Organic Sales - Suntips",
                          desc: "Zero ad spend campaign milestone",
                          date: "OCT 2024",
                          badge: "ACHIEVEMENT",
                          color: "bg-cyan-500",
                          badgeColor:
                            "bg-cyan-950/30 text-cyan-700 border-cyan-900/30",
                        },
                        {
                          title: "Kerala Election 2026 Dashboard",
                          desc: "Broadcast - quality live results platform",
                          date: "JAN 2026",
                          badge: "LAUNCH",
                          color: "bg-pink-600",
                          badgeColor:
                            "bg-pink-950/30 text-pink-700 border-pink-900/30",
                        },
                      ]
                  ).map((item, i) => (
                    <motion.div
                    ...
                    >
                      ...
                      {/* Content */}
                      <div className="flex-1">
                        <h4 className="text-sm font-bold text-white mb-0.5 tracking-tight group-hover:text-white transition-colors">
                          {item.title}
                        </h4>
                        <p className="text-[10px] text-gray-500 font-medium group-hover:text-gray-400 transition-colors uppercase tracking-tight">
                          {item.desc}
                        </p>
                        <span className="text-[9px] text-gray-600 font-bold tracking-[0.1em] mt-1.5 block uppercase">
                          {item.date}
                        </span>
                      </div>
                      ...
                      <div
                        className={cn(
                          "px-5 py-1.5 rounded-full border text-[9px] font-black uppercase tracking-widest sm:min-w-[124px] text-center self-start sm:self-auto mx-auto sm:mx-0",
                          item.badgeColor,
                        )}
                      >
                        {item.badge}
        ...
        >
          {!isAlenOrSham && (
            <GlassCard variant="purple" className="space-y-4 !p-4">
              {[
                {
                  icon: <Users className="w-4 h-4 text-accent-red" />,
                  label: "Department",
                  value: user.department || "Leadership & Strategy",
                },
                {
                  icon: <Clock className="w-4 h-4 text-accent-red" />,
                  label: "Joined",
                  value: user.doj
                    ? new Date(user.doj).toLocaleDateString("en-US", {
                        year: "numeric",
                ...
                },
                {
                  ...
                  label: "Location",
                  value: user.location || "Kochi, Kerala",
                },
                {
                  icon: <GraduationCap className="w-4 h-4 text-accent-red" />,
                  label: "Education",
                  value:
                    user.education && user.education.length > 0
                ...
                <div
                  key={i}
                  className="flex items-center gap-4 group border-b border-white/5 last:border-0 pb-4 last:pb-0"
                >
                  ...
                  <div>
                    <div className="text-[9px] text-gray-500 font-semibold uppercase tracking-[0.2em] mb-0.5 opacity-60">
                      {item.label}
                    </div>
            ...
            <GlassCard variant="purple" className="!p-4">
              ...
              <div className="space-y-4">
                {liveProjects.map((p, i) => (
                  <div
                    key={i}
                    className="p-4 rounded-2xl border border-white/5 bg-white/2 hover:bg-white/5 transition-all group relative"
                  >
                    <div className="flex justify-between items-start mb-2">
                      <span className="text-[8px] text-orange-600 font-bold tracking-widest uppercase">
                        {p.label}
                      </span>
                    ...
                    <h4 className="text-[11px] font-bold text-white mb-3">
                      {p.title}
                    </h4>
            ...
            <div className="space-y-4">
              ...
              {[
                ...
                const formattedPrice = hasPrice
                  ? session.price.startsWith("₹")
                    ? session.price
                    : `₹${session.price}`
                  : "";
                const titleText = hasPrice
                  ? `${session.time} - ${formattedPrice}`
                  : `${session.time} -`;
                ...
                return (
                  ...
                  >
                    <h4 className="text-3xl font-black text-white tracking-tight mb-0.5">
                      {titleText}
                    </h4>
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest opacity-60 mb-6">
                      Strategy Session
                    </p>
            ...
            <GlassCard variant="purple">
              ...
              <div className="flex flex-wrap gap-2.5">
                {(user && Array.isArray(user.hobbies) && user.hobbies.length > 0
                  ...
                  <span
                    key={idx}
                    className="btn-typo text-gray-400 px-5 py-2 rounded-full border border-white/10 hover:border-brand-purple/60 hover:text-white transition-all cursor-default uppercase bg-white/5"
                  >
                    {h}
                  </span>
                ))}
                ...
                {recentActivities.map((act, i) => (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <div
                        className={cn("w-1.5 h-1.5 rounded-full", act.color)}
                      />
          ...
          {!isAlenOrSham && (
            ...
            >
              ...
              <span className="text-xs md:text-sm font-medium text-white tracking-wide uppercase">
      ...
      {((user &&
        Array.isArray(user.workShowcase) &&
        user.workShowcase.length > 0) ||
        showStaticFallback) && (
        <motion.section
        ...
        >
          <div className="mb-10 flex items-center gap-3 border-b border-white/5 pb-6">
            ...
            <div>
              <span className="text-[10px] font-black tracking-[0.3em] text-white/30 uppercase mb-1 block">
              ...
              : [
                  {
                    ...
                    description:
                      "Created high-performing content strategy that increased brand reach and conversions.",
                    images: [
                      "https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/TeamPage/Rectangle%2098.png",
                      "https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/TeamPage/Rectangle%2098.png",
                      "https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/TeamPage/Rectangle%2098.png",
                  ...
                  },
                  {
                    ...
                    description:
                      "Managed social media accounts and grew audience organically with content strategy.",
                    images: [
                      "https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/TeamPage/Rectangle%2098.png",
                      "https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/TeamPage/Rectangle%2098.png",
                      "https://pub-dbc24446d37a40aeb1dfdd10992cd2d9.r2.dev/TeamPage/Rectangle%2098.png",
        ...
        >
          {hasInnovations && (
            <motion.div
              ...
              className={cn(
                "space-y-8",
                hasRightColumn ? "lg:col-span-8" : "lg:col-span-12",
              )}
            >
              ...
              <div className="space-y-6">
                {user &&
                Array.isArray(user.innovations) &&
          ...
          {hasRightColumn && (
            <motion.div
              ...
              className={cn(
                "space-y-8",
                hasInnovations ? "lg:col-span-4" : "lg:col-span-12",
              )}
            >
              {hasPodcasts && (
                <>
                    ...
                    user.podcasts.length > 0
                      ? user.podcasts.map((pod, idx) => (
                          ...
                          >
                            <div
                              className={cn(
                                "w-20 h-20 rounded-2xl flex items-center justify-center transition-all text-white shadow-2xl relative overflow-hidden bg-brand-purple",
                                idx % 3 === 0
                                  ? "bg-[#8A0699]"
                                  : idx % 3 === 1
                                    ? "bg-[#22C55E]"
                            ...
                            <div>
                              <div className="text-[10px] font-semibold text-brand-purple mb-1 uppercase tracking-widest">
                                EP{" "}
                                {String(pod.episodeNo || idx + 1).padStart(
                                  2,
                                  "0",
                                )}
                              </div>
                              <h4 className="text-sm font-semibold text-white mb-2 leading-tight group-hover:text-brand-purple transition-colors">
                                {pod.title ||
                                  "How API Marketing Rewrites the Rules"}
                              </h4>
                      ...
                      : [1, 2, 3].map((i) => (
                          <div
                            key={i}
                            className="glass-card-purple flex items-center gap-5 group cursor-pointer p-5 hover:scale-[1.02] transition-all border-white/10"
                          >
                            <div
                              className={cn(
                                "w-20 h-20 rounded-2xl flex items-center justify-center transition-all text-white shadow-2xl relative overflow-hidden",
                                i === 1
                                  ? "bg-[#8A0699]"
                                  : i === 2
                                    ? "bg-[#22C55E]"
                  ...
                  <div className="space-y-6">
                    ...
                    user.events.length > 0
                      ? user.events.map((ev, idx) => (
                          <div
                            key={idx}
                            className="glass-card flex items-center gap-6 p-5 hover:bg-white/5 transition-all border-white/10 group"
                          >
                            <div
                              className={cn(
                                "w-16 h-16 rounded-2xl flex flex-col items-center justify-center shadow-lg transition-transform group-hover:-rotate-6",
                                ev.category === "purple"
                            ...
                            >
                              <span className="text-2xl font-semibold leading-none mb-0.5">
                                {ev.date || "14"}
                              </span>
                            ...
                            <div className="flex-1">
                              <h4 className="text-xs font-semibold text-white mb-1 group-hover:text-brand-purple transition-colors uppercase tracking-tight">
                                {ev.title || "API Marketing Master Class"}
                              </h4>
                              <p className="text-[10px] text-gray-500 font-medium uppercase tracking-widest opacity-60">
                                {ev.details || "Free webinar - Kerala owners"}
                              </p>
                            </div>
                            <div
                              className={cn(
