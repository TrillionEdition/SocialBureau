        ...
        <section className="w-full grid grid-cols-1 md:grid-cols-2 md:grid-rows-2 md:h-screen h-auto">
            {items.map((item, index) => (
                <Link
                    ...
                    to={item.link}
                    className={`relative flex flex-row md:flex-col items-center md:items-left justify-between md:justify-center ${item.bg} ${item.text} overflow-hidden group p-4 h-36 sm:h-44 md:h-auto`}
                >
                    {/* TEXT */}
                    ...
                    <img
                        src={item.image}
                        alt={item.title}
                        className="static md:absolute bottom-0 right-0 w-1/3 md:w-1/2 max-w-xs object-contain opacity-90 transition-transform duration-700 group-hover:scale-105"
                    />
                </Link>
            ))}
        </section>
